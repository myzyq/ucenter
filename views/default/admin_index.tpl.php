			...
			<table id='admin_data' class="datalist fixwidth" >
				<tbody>	
					...
					<?php foreach($data as $item) {
						...
						echo "<td >{$isband}</td>"; 
						$ips = str_replace("\n", ";", $item['ip_band']);
						echo "<td ><a href='?m=member&a=index&keys[id]={$item['id']}'>员工</a> | <a href='?m=user&a=edit&id={$item['id']}'>修改</a> | <a href='?m=user&a=reset_pwd&id={$item['id']}' onclick='return confirm(\"要重新生成密码吗？\");'>改密码</a></td>"; 
						echo "</tr>\n";
		}?>
					<tr class="nobg">
						<td>
