 ...
 * 常用函数
 * @author d-boy
 * $Id: common.fun.php 116 2010-04-12 01:44:06Z lunzhijun $
 */
...
function checkform($string) {
	if(is_array($string)) {
		foreach($string as $key => $val) {
			$string[$key] = mysql_escape_string($val);
		}
	} else {
...
function curr_user(){
	...
	list($_userid,$_password) = explode("|", $cookie);	
	$user = array("id" => $_userid, "password" => $_password);
	return $user;
}
