 ...
 * 分页类
 * @author d-boy
 * $Id: pager.class.php 4 2010-02-04 01:36:58Z lunzhijun $
 */
class Pager {
	var $page ; //当前页码
	var $total; //总记录数
	...
	function __construct($config = array()) {
		...
		//记算总页数
		$this->pageCount =   ceil($this->total/($this->pageSize * 1.0));  
	}
	...
	function exec_pager() {
		...
		if( (empty($this->total) || empty($url)) && !$always) return false; //没有总数或总数为0且不总输出，不输出
		
		if($url) $url = preg_replace("/(.*)([&?]page=[0-9]*)(.*)/i", "\\1\\3", $url);
		$s = strpos($url, '?') === FALSE ? '?' : '&';
	...
	function exec_ajax_pager() {
		...
		if( empty($this->total)  && !$always) return false; //没有总数或总数为0且不总输出，不输出
		...
		$pagenav .= "页码：<b><font color=red>$page</font>/{$this->pageCount}</b>&nbsp;&nbsp;<input type='text' name='page' id='page' class='page' size='2' onKeyDown=\"if(event.keyCode==13) 
					{ajaxPager(this.value); return false;}\"> 
					<input type='button' value='GO' class='gotopage' style='width:30px' 
