...
$res = mysql_query("SELECT id FROM `ucenter`.`user_base` where email = '".$Account."@gyyx.cn' limit 1",$con);
$result = mysql_fetch_row($res);

//检查  该用户是否有app登陆权限
if($result[0]){
	$user_id = $result[0];
	$res_app = mysql_query("SELECT * FROM `ucenter`.`app_user_ip` where user_id = '$user_id' and app_id = '$app_id' limit 1",$con);
